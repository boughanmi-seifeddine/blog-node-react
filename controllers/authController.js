...
/**
 * @description creates jwt token from payload and secret
 * @param id
 * @param secret
 * @param expiresIn
...
const addUserToken = async (user, refreshToken, next) => {
    try {
        const userToken = await UserToken.findOneAndDelete({userId: user._id});
        return await new UserToken({userId: user._id, token: refreshToken}).save();
    } catch (err) {
        // await User.deleteOne({ _id: user._id }) todo
...
exports.login = catchAsync(async (req, res, next) => {
    ...
    let {token, refreshToken} = createSendToken(user, res);
    // add user refresh token to userToken
    await addUserToken(user, refreshToken, next)
...
const verifyRefreshToken = async (refreshToken, next) => {
    try {
        ...
        console.log(decoded)
        if (!decoded) return next(new AppError('invalid refresh token', 404));
        return User.findById(decoded.id);
    } catch (err) {
        return next(new AppError('can not refresh token', 404));
    }

};

exports.refresh = catchAsync(async (req, res, next) => {
    if (!req.body.refreshToken) return next(new AppError('please provide a refresh token', 404));
    const user = await verifyRefreshToken(req.body.refreshToken, next)
    if (!user) return next(new AppError('user associated not found', 404));
    const {token, refreshToken} = createSendToken(user, res);
    await addUserToken(user, refreshToken, next)
    // Remove password from output
    user.password = undefined;
